...
st.markdown(
    ...
    <div style="text-align:center; padding:20px 0;
        ...
        <p style="font-family:'Poppins',sans-serif; font-size:0.7rem; font-weight:500;
                  color:#8a8a9a; text-transform:uppercase; letter-spacing:0.08em;
...
with st.expander("📊 Info Keuangan Bulan Ini", expanded=True):
    ...
    i4.metric("💳 Pengeluaran Tetap/Bulan", rupiah(pengeluaran_tetap))
...
# ==========================================
# BATAS HARIAN (ON / OFF DAN TERSIMPAN)
# ==========================================
df_pg_batas = df_pg[
    (df_pg["Jenis"] == "SETTING") &
...
if aktif_batas != batas_aktif_default:
    ...
    row_setting = {
        ...
        "Nominal": "0",
        "Periode": "-",
        "Status": "AKTIF" if aktif_batas else "NONAKTIF",
        "Bulan_Bayar": "-",
        "Counter_Bayar": "0"
    }

    df_pg_update = pd.concat(
        [df_pg_update, pd.DataFrame([row_setting])],
        ignore_index=True
    )
...
if aktif_batas:
    ...
    batas_harian = saldo_siap / sisa_hari if sisa_hari > 0 and saldo_siap > 0 else 0.0
    sisa_batas = batas_harian - keluar_hari_ini

    b1, b2 = st.columns(2)
    b1.metric("🎯 Batas Harian", rupiah(batas_harian))
    b2.metric("💵 Sisa Batas Hari Ini", rupiah(sisa_batas))

    b3, b4 = st.columns(2)
    b3.metric("📤 Keluar Hari Ini", rupiah(keluar_hari_ini))
    ...
    if batas_harian <= 0:
        ...
        if keluar_hari_ini <= 0:
            st.success(f"✅ Belum ada pengeluaran hari ini. Batas penuh {rupiah(batas_harian)}")
        else:
            persen_sisa = (sisa_batas / batas_harian) * 100 if batas_harian > 0 else 0
            if sisa_batas < 0:
                st.error(f"🚨 Melebihi batas harian sebesar {rupiah(abs(sisa_batas))}")
            elif persen_sisa < 20:
                st.error(f"❌ Sisa batas tinggal {rupiah(sisa_batas)} ({persen_sisa:.0f}%)")
            elif persen_sisa < 50:
                st.warning(f"⚠️ Sisa batas {rupiah(sisa_batas)} ({persen_sisa:.0f}%)")
            else:
                st.success(f"✅ Masih aman. Sisa {rupiah(sisa_batas)} ({persen_sisa:.0f}%)")
