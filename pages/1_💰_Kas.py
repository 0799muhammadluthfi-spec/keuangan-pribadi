...
# RESET COUNTER
# ==========================================
if "KEUANGAN_rc" not in st.session_state:
    st.session_state["KEUANGAN_rc"] = 0

def reset_form():
    st.session_state["KEUANGAN_rc"] += 1

# ==========================================
# NAVIGATION
# ==========================================
render_top_nav(active="KEUANGAN")

# ==========================================
# LOAD DATA
# ==========================================
df_KEUANGAN = load_data(conn, WS_KEUANGAN)
df_KEUANGAN = pastikan_kolom(df_KEUANGAN, KOLOM_KEUANGAN)
...
# RINGKEUANGANAN
# ==========================================
last_seluruh, last_KEUANGAN, last_atm, last_shopee = get_last_saldo(df_KEUANGAN)
total_masuk, total_keluar = hitung_ringKEUANGANan(df_KEUANGAN)
...
r2c1.metric("💵 Tangan", rupiah(last_KEUANGAN))
...
# ==========================================
level, pesan = cek_warning_harian(df_KEUANGAN, df_pg)
if level == "bahaya":
    st.error(pesan)
elif level == "warning":
    st.warning(pesan)
...
if cek_selisih:
    ...
    st.metric("💵 KEUANGAN di Tangan (Sistem)", rupiah(last_KEUANGAN))

    KEUANGAN_fisik = st.number_input(
        "Input KEUANGAN Fisik di Tangan",
        value=float(last_KEUANGAN),
        step=1000.0,
        format="%.0f",
        key="input_cek_fisik"
    )

    selisih = last_KEUANGAN - KEUANGAN_fisik
    ...
    if st.button("💾 Simpan Hasil Pengecekan", type="primary",
        ...
        row_cek = {
            ...
            "KEUANGAN_Fisik": fmt_nominal(KEUANGAN_fisik),
            "KEUANGAN_Sistem": fmt_nominal(last_KEUANGAN),
...
with tab1:
    ...
    if jenis == "MASUK":
        ...
        tujuan = st.selectbox(
            "🎯 Tujuan Anggaran",
            ["SALDO KEUANGAN (DI TANGAN)", "UANG DI ATM", "UANG DI SHOPEE"],
            key=f"k_{rc}_tujuan"
        )
    ...
    # ── KELUAR ──
        ...
        sumber = st.selectbox(
            "💳 Sumber Anggaran",
            ["UANG KEUANGAN (DI TANGAN)", "UANG DI ATM", "UANG DI SHOPEE"],
            key=f"k_{rc}_sumber"
        )

        if sumber == "UANG KEUANGAN (DI TANGAN)" and nominal > last_KEUANGAN and nominal > 0:
            st.warning(f"⚠️ Melebihi saldo KEUANGAN ({rupiah(last_KEUANGAN)})")
    ...
    # ── HITUNG SALDO BARU ──
    if jenis == "MASUK":
        if tujuan == "SALDO KEUANGAN (DI TANGAN)":
            new_KEUANGAN = last_KEUANGAN + nominal
            new_atm = last_atm
            new_shopee = last_shopee
        elif tujuan == "UANG DI ATM":
            new_KEUANGAN = last_KEUANGAN
        ...
        if sumber == "UANG KEUANGAN (DI TANGAN)":
            new_KEUANGAN = last_KEUANGAN - nominal
            new_atm = last_atm
            new_shopee = last_shopee
    ...
    total_baru = new_KEUANGAN + new_atm + new_shopee
    ...
    if nominal > 0:
        ...
        h1.metric("💵 Tangan", rupiah(new_KEUANGAN))
    ...
    with b1:
        simpan = st.button(
            ...
            key="btn_simpan_KEUANGAN"
        )
    with b2:
        st.button(
            "🔄 Reset",
            use_container_width=True,
            key="btn_reset_KEUANGAN",
            on_click=reset_form
        )
    ...
    if simpan:
        ...
        if not keterangan:
            ...
            next_no = get_next_no(df_KEUANGAN)
            new_row = {
                "No": str(next_no),
                "Tanggal": tanggal,
                ...
                "Sisa_KEUANGAN_Di_Tangan": fmt_nominal(new_KEUANGAN),
                "Sisa_ATM": fmt_nominal(new_atm),
                "Sisa_Shopee": fmt_nominal(new_shopee),
                "Sisa_KEUANGAN_Seluruh": fmt_nominal(total_baru),
                "Catatan": "-"
            }

            df_baru = pd.concat(
                [df_KEUANGAN, pd.DataFrame([new_row])],
                ignore_index=True
            )
            if safe_update(conn, WS_KEUANGAN, df_baru):
...
with tab2:
    ...
    with c_btn2:
        tombol_refresh("ref_KEUANGAN_data")

    df_valid = df_KEUANGAN[df_KEUANGAN["No"] != "-"].copy()
    ...
    if df_valid.empty:
        ...
        kolom = [
            ...
            "Sisa_KEUANGAN_Di_Tangan", "Sisa_ATM",
            "Sisa_Shopee", "Sisa_KEUANGAN_Seluruh"
        ]
